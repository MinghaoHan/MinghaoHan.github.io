...
photo = {
    ...
    render: function (page, data) {
        ...
        for (var i = begin; i < end && i < data.length; i++) {
           ...
           var imgDes = imgName.split('/').slice(-1);
           imageSize = data[i].split(' ')[0];
           imageX = imageSize.split('.')[0];
           imageY = imageSize.split('.')[1];
           li += '<div class="card" style="width:' + imageWidth + 'px" >' +
                    '<div class="ImageInCard" style="height:'+ imageWidth * imageY / imageX + 'px">' +
                      '<a data-fancybox="gallery" href="' + imgNameWithPattern + '" data-caption="' + imgDes + '" title="' +  imgDes + '">' +
                        '<img src="' +  imgNameWithPattern + ' " data-src="' + imgPath + imgNameWithPattern + ' " data-loaded="true">' +
                      '</a>' +
                    '</div>' +
                  '</div>'
        }
